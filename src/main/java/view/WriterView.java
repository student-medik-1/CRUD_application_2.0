...
public class WriterView extends AbstractView {
    ...
    private static final String WRITER_REGION_NAME = "region";
    ...
    void createNewRecord(String[] command) {
        ...
        if (command.length > 3) {
            ...
            try {
                writerController.create(command[firstName], command[lastName],
                        new Region(command[nameRegion]));
    ...
    void editRecord(String[] command) {
        ...
        if (command.length > 4) {
            ...
            try {
                writerController.update(Long.valueOf(command[id]), command[firstName], command[lastName],
                        new Region(command[regionName]));
    ...
    void helpMe() {
        ...
        System.out.println("Данные про писателей будет выводиться в виде:  \n" +
                ...
                EDIT_BY_ID + " " + WRITER_FIRST_NAME + " " + WRITER_LAST_NAME + " " + WRITER_REGION_NAME +
                "  ID FIRST_NAME LAST_NAME REGION_NAME- изменить данные писателя по ID \n" +
